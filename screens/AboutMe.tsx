...
function AboutMe() {
  ...
  const AnimatedStatusDots: React.FC<{index: number}> = ({index}) => {
    ...
    if (currentDotIndex === index) {
      dotWidth.value = withTiming(100, {duration: 6000}, completed => {
        if (completed) {
          runOnJS(setCurrentDotIndex)(
            currentDotIndex === index - 1 ? 0 : currentDotIndex + 1,
          );
        }
      });
    }
    ...
    return (
      <View style={styles.animatedDot}>
        <Animated.View
          style={[
            {backgroundColor: 'white', flex: 1, borderRadius: 20},
            animatedStyle,
          ]}
        />
      </View>
  ...
  const handleStatusChange = (e: any) => {
    ...
    if (horizontalPosition > width / 2) {
      setCurrentDotIndex(
        currentDotIndex === slides.length - 1 ? 0 : currentDotIndex + 1,
      );
    } else {
      setCurrentDotIndex(
        currentDotIndex === 0 ? currentDotIndex : currentDotIndex - 1,
      );
    }
  };
    ...
    <View style={{backgroundColor: 'lightgreen', flex: 1}}>
      ...
      <View style={styles.animatedDotsContainer}>
        {Array(slides.length)
          .fill(null)
          .map((_, index) => (
            <AnimatedStatusDots key={index} index={index} />
          ))}
      </View>
  ...
  return (
    <View style={styles.presentationTextContainer}>
      <Text style={styles.presentationText}>
        Hi😎! I'm Jeremiah. I'm a frontend developer
      </Text>
    </View>
  );
};
const Slide3 = () => {
  return (
    <View style={styles.presentationTextContainer}>
      ...
      <Text style={[styles.presentationText, {fontSize: 30}]}>
        I love building beautiful and interesting mobile and web applications
      </Text>
    </View>
  );
};
...
const Slide5 = () => {
  return (
    <View style={styles.presentationTextContainer}>
      ...
      <Text style={styles.presentationText}>
        I love meeting new people and eating good food.
      </Text>
    </View>
  );
};
...
const Slide6 = () => {
  return (
    <View
      style={[
        styles.presentationTextContainer,
