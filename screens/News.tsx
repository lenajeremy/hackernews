...
function NewsScreen() {
  ...
  function _renderNews({item: item}: {item: number}) {
    return <NewsType type={StoryType.top} item={item} />;
  }
    ...
    return (
      <SafeAreaView
            ...
            <Pressable
              onPress={() =>
                setNewsType(newsType =>
                  newsType === StoryType.new ? StoryType.top : StoryType.new,
                )
              }>
              <Ionicon
                size={26}
        ...
        <FlatList
          ...
          ListFooterComponent={
            <ActivityIndicator style={{marginBottom: 30}} color="#ec6333" />
          }
        />
      </SafeAreaView>
    );
  ...
  if (loaded) {
    return (
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate(RouteNames.storyScreen, {story})}>
        <View style={styles.loadingNewsContainer}>
          <View
            ...
            <Text style={styles.posterText}>
              {story.poster.charAt(0).concat(story.poster.charAt(1))}
            </Text>
          </View>
          <View style={{width: '80%'}}>
            <Text
              style={[styles.topText, {color: isDarkMode ? 'white' : 'black'}]}>
              {story?.text}
            </Text>
            <View style={{flexDirection: 'row'}}>
              <Text
                style={[
                  styles.storiesDetails,
